...
define('mail/Message', [
	 ...
	 * @return {Object}
	 */
	Message.compose = function (options) {
		return {
			uid: uuid().replace(/-/g, ''),
			model: new this(),
			options: util.extend({
			...
			_upload: function () {
				var item = !this._uploading && this._queue.shift();
				...
				if (item) {
					...
					FileAPI.upload({
						url: '..',
						data: {message_id: this.uid},
						files: {file: item.file},
						onfileprogress: function (evt) {
							item.options.onprogress(evt, item.file);
						},
						onfilecomplete: function (err, xhr) {
							xhr = RPC.from(err ? {status: 500, body: err} : xhr);
							item[xhr.isOK() ? 'resolve' : 'reject'](xhr);
...
define('node_modules/require-css/normalize',[],function() {
...
define('node_modules/require-css/css.js',[],function() {
...
define('node_modules/require-css/css.js!ui/nav/nav',[],function(){});
(function (factory){
  "use strict";
  window["xtpl"] = factory();
  if( typeof define === "function" && define.amd ){
    define('xtpl',[],function (){ return xtpl; });
  } else if( typeof module != "undefined" && typeof module.exports != "undefined" ){
...
define("utils", function(module, exports, require){"use strict";
...
module.exports = utils = {
	...
	toCamelCase: function (str) {
		return str.replace(/-(.)/g, _toUpperCase);
	},
	 ...
	 * @param {*} [scope]
	 */
	each: function(input, iterator, scope){
		if( input ){
			var i = 0, n = input.length;

			if( n >= 0 && (0 in input) ){
				for( ; i < n; i++ ){
					iterator.call(scope, input[i], i, input);
				}
			} else {
				for( i in input ){
	 ...
	 * @returns {Array}
	 */
	map: Array.map || function (array, iterator, scope){
		var res = [];
		for( var i = 0, n = array.length; i < n; i++ ){
			res.push(iterator.call(scope, array[i], i, array));
		}
		return	res;
	},
	 ...
	 * @returns {Array}
	 */
	filter: Array.filter || function (array, iterator, scope){
		var res = [];
		for( var i = 0, n = array.length; i < n; i++ ){
			if( iterator.call(scope, array[i], i, array) ){
				res.push(array[i]);
			}
		}
		return	res;
	 ...
	 * @returns {Number}
	 */
	inArray: Array.indexOf || function (array, item){
		var i = array.length;
		while( i-- ){
			if( array[i] === item ){
				return	i;
	 ...
	 * @returns {boolean}
	 */
	notEqual: function _notEqual(actual, expected){
		var key, eType = typeof expected;

		if( typeof actual !== eType && eType === 'object' ){
			return	true;
		}
		else if( eType === 'object' && actual !== null && expected !== null ){
			if( actual !== expected ){
				key = expected.length;

				if (key >= 0) { // Array
					if( actual.length !== key ){
						return true;
					}
					else {
						while( key-- ){
	 ...
	 * @returns {boolean}
	 */
	simpleNotEqual: function _notEqual(actual, expected){
		var key, eType = typeof expected;
	...
	print: function (){
		console.log(utils.map(arguments, function (val){
			return	val ? JSON.stringify(val).toString().replace(/^"|"$/g, '') : val;
		}));
	},

	error: function (err, file, line){
//		debugger;
		console.log(err.message+'\nline: '+(line || err.line || err.lineNumber)+'\nfile: '+(file || err.file || err.filename));
	},
	...
	throttle: function (fn, delay, thisArg){
		var
			  pid
			, ts
			, now
			, args
			, trailingCall = function (){ fn.apply(thisArg, args); }
		;
		...
		return function (){
			...
			if( pid === void 0 ){
				ts = now;
				pid = setTimeout(trailingCall, delay);
			}
			else if( now - ts >= delay ){
				clearTimeout(pid);
				pid = void 0;
				trailingCall();
			}
...
if( utils.isClient ){
	utils.support.touch = (function (){
		var el = document.createElement('div');
		return	'ontouchstart' in el;
	})();


	if( window.jQuery ){
		var tapEvent = utils.support.touch ? 'touchstart' : 'click';
		jQuery.event.special.tap = { delegateType: tapEvent, bindType: tapEvent };
	}
}
});define("Node", function(module, exports, require){(function () {
	var
		  utils = require('utils')
		, _each = utils.each
		, _shorty	= utils.shortTags
	;
	...
	function Node(name, attrs){
		this.name		= name || '#';
		this.shorty		= _shorty[name] !== void 0;
		this.value		= '';
		this.attrs		= attrs || {};
		this.length		= 0;
		this.children	= [];
	...
	Node.prototype = {
		...
		bind: function (attr){
			this.binding.push(attr);
		},

		on: function (name, expr, selector){
			if( this.events === void 0 ){
				this.events = [];
			}
			this.events.push(name.replace(/^.{1,3}[:-]+/, ''), expr, selector);
		},

		bindDecl: function (name, expr){
			if( this.decl === void 0 ){
				this.decl = {};
			}
			this.decl[name] = expr;
		...
		index: function (){
			var children = this.parent.children, i = children.length;
			while( i-- ){
				if( children[i] === this ){
					return	i;
				}
		...
		append: function (node, before){
			var children = this.children;

			if( before === void 0 ){
				children.push(node);
			} else {
				children.splice(before.index(), 0, node);
			}

			this.first	= children[0];
			this.last	= children[(this.length = children.length) - 1];
			node.parent	= this;
		...
		remove: function (){
			var parent = this.parent, children = parent.children, idx = this.index();
			if( idx !== -1 ){
				children.splice(idx, 1);
			}
			parent.first	= children[0];
			parent.last		= children[(parent.length = children.length) - 1];
	//			delete this.parent;
			return	this;
		...
		find: function (name){
			var nodeList = [], recursion = name.charAt(0) != '>';

			name = name.replace('>', '');
			_each(this.children, function _find(node){
				if( node.name === name || name === '*' ){
					nodeList.push(node);
				}

				if( recursion ){
					_each(node.children, _find);
				}
			});
		...
		wrap: function (name, attrs){
			var wrap = newNode(name, attrs);
			wrap.file = this.file;
			wrap.line = this.line;
			this.parent.append(wrap, this);
			wrap.append(this.remove());
			return	wrap;
		},

		innerWrap: function (name, attrs){
			var node = newNode(name, attrs), children = this.children, i = 0, n = children.length;
			this.children = [];
			for( ; i < n; i++ ){
		...
		replace: function (node){
			var parent = this.parent, array = [].concat(node), i = 0, n = array.length;
			this.remove();
			for( ; i < n; i++ ){
				node = array[i];
				if( typeof node === 'string' ){
					parent.appendText(node);
				} else {
					parent.append(node);
				}
			}
		...
		appendText: function (text){
			if( text !== '' ){
				var last = this.last;
				if( last === void 0 || last.name !== '#text' ){
					this.newChild('#text').value = text;
				}
				else {
					last.value += text;
				}
		...
		newChild: function (name, attrs){
			var child = newNode(name, attrs);
		...
		each: function (iterator){
			iterator(this, this.attrs);
			_each(this.children, function (node){
				node.each(iterator);
			});
		},
		...
		clone: function (){
			...
			_each(this, function (val, key){
				if( key === 'children' ){
					_each(val, function (child){
						node.append( child.clone() );
					});
				} else if( /first|last/.test(key) ){
					/* nothing */
				} else {
					node[key] = utils.simpleClone(val);
				}
			});
		...
		toString: function (format, pad){
			pad = format && pad || '';
			format = format ? '\n' : '';

			var str = pad + this.value;

			_each(this.children, function (node){
				str += node.toString(format, '  '+pad);
			});


			if( this.name.charAt(0) != '#' ){
				var tmp = format + pad+'<'+this.name, attrs = [], name, val;
				...
				str = tmp + (this.shorty && !this.length ? '/>' : '>'+ str + format + pad +'</'+ this.name +'>');
			}
...
});define("xparser", function(module, exports, require){(function (){
	"use strict";
	...
	var
		...
		, _code		= function (ch){ return ch.charCodeAt(0); }
		, _char		= function (code){ return String.fromCharCode(code); }
		, _stateId	= 0

		, cmd_n	= _code('\n')

		, c_lb	= _code('{') // left brace
		, c_rb	= _code('}') // right brace

		, c_lp	= _code('(') // left parenthesis
		, c_rp	= _code(')') // right parenthesis

		, c_lbr	= _code('[') // left bracket
		, c_rbr	= _code(']') // right bracket
		...
		, s_id			= ++_stateId
		, s_tag			= ++_stateId
		, s_tag_attrs	= ++_stateId
		, s_block		= ++_stateId
		, s_class		= ++_stateId
		, s_text		= ++_stateId
		, s_attr		= ++_stateId
		, s_attr_val	= ++_stateId
		, s_attr_prop	= ++_stateId
		...
		, _rpropPx	= /(width|height|left|right|top|bottom|font-size|text-indent)$/

		, _tagName	= 'a abbr acronym address applet area article aside audio b base basefont bdi bdo big blockquote body br canvas caption center cite code col colgroup command datalist dd del details dfn dialog dir div dl dt em embed fieldset figcaption figure font footer form frame frameset h1 to h6 head header hgroup hr html i iframe img input ins kbd keygen label legend li link map mark menu meta meter nav noframes noscript object ol optgroup option output p param pre progress q rp rt ruby s samp script section select small source span strike strong style sub summary sup table tbody td textarea tfoot th thead time title tr track tt u ul var video wbr'.split(' ')

		, _keywords	= {
			'if': function (expr) {
				return newNode('x:if', {test: expr});
			},

			'scope': function (expr) {
				return newNode('x:scope', {ctx: expr});
			},

			'bind': function (expr) {
				return newNode('x:bind', {state: expr === 'true' ? true : (expr === 'false' ? false : expr)});
			},

			'else': function (expr, root){
				var els	= newNode('x:else');
				els.shorty = true;
				if( /^if\b/.test(expr) ){
					els.attrs['if'] = expr.substr(2).trim();
				}
				return root.last.append(els);
			},

			'elseif': function (expr, root){
				return	_keywords['else']('if '+expr, root);
			},

			'for': function (expr, root){
				return	_keywords.each(expr, root);
			},

			'each': function (str, root){
				var expr = str.match( /^\(?(\w+)(?:\s|,)\s*(\w+)?\)?\s*in\s*([^\|]+)(\|.+)?/i );

				if (!expr) {
					root.exception('for ' + str + ' — syntax error');
				}

				return	newNode('x:each', {
					  data: expr[3]
					, as: expr[2] || expr[1]
					, key: expr[2] ? expr[1] : '$index'
					, filter: /^\|/.test(expr[expr.length-1]) ? expr.pop().substr(1) : void 0
				});
			},
			'include': function (src) {
				return newNode('x:include', { src: src });
			}
		}
	;


	_each(_tagName, function (name){
		_tagName[name] = 1;
	});



	var xparser = function _parse(input, options, onTag){
		onTag = onTag || {};
		options = options || {};
		...
		var
			...
			, len = input.length
			, tmp // tmp var
			, code // current char code
			, prev // prev char code
			, root = newNode('#root')
			, line = options.line || 1 // file line
			, file = options.file
			, lineIdx = 0 // line position index
			, state // parse state
			, shorty // short tag
			, name // open node name
			, attr // attribute name
			, prop // attr-prop
			, value // attrbiute value
			, expr // expression
		;
		 ...
		 * @param {Number} [offset]
		 */
		function _throw(message, offset){
			var end = input.indexOf('\n', idx), pos = (idx - lineIdx) + 1;
			...
			var error = new Error([
				'Line ' + line + ':' + pos + ' in ' + file,
				(message || 'Syntax error'),
				input.substring(lineIdx,  end).replace(/\t/g, ' '),
				(new Array(pos + (offset|0))).join('-') + '^'
			].join('\n'));
		...
		function _parseExpr(expr, toObj){
			var res;

			try {
				expr = typeof expr === 'string'
							? expr.replace(/(\w)->(\w)/g, '$1.xxx.$2') // replace, else "Syntax error"
							: expr
						;
				res = new Function('ctx,Soul,name', 'return '+expr)();
			} catch( err ){
				expr = expr.replace(/(\w)\.xxx\.(\w)/g, '$1->$2'); // revert

				if( err instanceof ReferenceError || err instanceof TypeError ){
					return	toObj ? { expr: expr, str: expr } : '{{'+ expr +'}}';
				}
				else {
					_throw(err.message+'\n---\n'+expr+'\n---');
				}
			...
			return	toObj ? { value: expr, str: expr, result: res } : res;
		}


		function _trace(label){
			console.log(label+'['+state+']', '>>', [input.substring(last, idx), _char(code)]);
		}


		function _onText(value){
			var offset = 0;

			value.replace(/\{([{*])(.*?)[*}]\}/g, function (match, mode, expr, idx) {
				if (idx > 0) {
					root.newChild('#text').value = value.substring(offset, idx);
				}
				offset = idx + match.length;

				match = root.newChild('x:value');
				match.value = expr;

				if (mode === '*') {
					match.attrs.output = 'text'
				}
			});

			root.newChild('#text').value = value.substring(offset);
		}


		function _getAttrVal() {
			var slash, value, parenthesis = 0;
			...
			while (idx < len) {
				...
				if (
					code === c_quote ||
					code === c_squote ||
					(code === c_bslash && !/["')]/.test(_char(prev)))
				) {
					// Skip "string" or /regexp/
					idx++;
					prev = code;
					...
					if( code === c_comma || code === c_rb || code === c_nl ){ // ","  "}" \"n"
						// end of attr value
						value = input.substring(last, idx).trim().replace(/[;,]$/, '');
						last = idx + 1;
						idx--;
						return    value;
					}
		...
		function _getLine(offset){
			...
			return	input.substring(last, idx);
		}


		function _onTag(node){
			// todo: "if" -- fix me
			...
			var fn = onTag[node.name], res;
			if( fn !== void 0 ){
				res = typeof fn === 'string' ? fn : fn(node, node.attrs);
		...
		function _close(node, withoutRecursion) {
			var attrs = {};

			_each(node.attrs, function (values, attr) {
				var value = values,
					nodeName = node.name;

				if (typeof values === 'string') {
					if (!/^x[:-](if|each|scope|bind)/.test(nodeName) && !/^(x|ng|on)-/.test(attr)) {
						value = _parseExpr(value);
					}
				}
				else if (values instanceof Object) {
					value = '';

					_each(values, function (term, prop) {
						term = _parseExpr(term, true);

						// "class" attr
						if (attr === 'class') {
							if (term.expr !== void 0) {
								prop = prop.replace(/^([^'"])/, '"$1').replace(/([^'"])$/, '$1"');
								value += ' {{' + term.expr + ' && ' + prop + ' || ""}}';
							}
							else if (term.result) {
								value += ' ' + prop.trim().replace(/(^["']|["']$)/g, '');
							}
						}
						...
						// "style" attr
							...
							value += prop + ':'
									+ (term.expr !== void 0 ? '{{' + term.expr + '}}' : term.result)
									+ (_rpropPx.test(prop) &&
										(term.expr
											? !/(\+\s*["'](%|[a-z]{2,3})['"]|%)$/.test(term.str)
											: /^\d+(\.\d+)?$/.test(term.str)
										) ? 'px' : ''
									) + ';';
						}
						...
						// "other" attr
							...
							value = void 0;
							attrs[attr + '-' + prop] = (term.expr !== void 0 ? '{{' + term.expr + '}}' : term.result);
						}
					});
				}

				// Обрабатываем `&` для bem
				if (attr === 'class' && value.indexOf('&') > -1) {
					var match,
						block = node,
						className;

					if (/&_[0-9a-z]/.test(value)) {
						match = value.match(/(?:^|\s)((?:&__)?[0-9a-z-]+)/);
						value = value.replace(/&(_[a-z])/g, match[1] + '$1');
						match = null;
					}

					while (block = block.parent) {
						className = block.attrs['class'];

						if (className) {
							if (className.match) {
								match = className.match(/(?:"|\s)([0-9a-z-]+)/i);
							}
							else if (className instanceof Object) {
								Object.keys(className).some(function (name) {
									match = name.match(/(?:"|\s)([0-9a-z-]+)/i);
									return !!match;
								});
							}
						}
						...
						if (match) {
							value = value.replace(/(^|\s|["'])&([-_]+[0-9a-z_-])/g, '$1' + match[1] + '$2').trim();
							break;
						}
					}
				}
			...
			if (!node.parent) {
				_throw('Too many close');
			}

			_onTag(node);

			if (node.parent.name == 'x:decl') {
				node = node.parent;
				_onTag(node);
			}

			return !withoutRecursion && node.parent.single ? _close(node.parent) : node.parent;
		}


		function _badChar(){
			if( code === c_nl ){
				lineIdx = input.lastIndexOf('\n', idx-1) + 1;
				idx++;
			}
			_throw("Bad character: "+code);
		}


		function _getLex(offset){
			var lex = input.substring(last+(offset|0), idx);
			last = idx;
			return	lex;
		}


		function _addKeyword(name){
			while( idx < len ){
				code = input.charCodeAt(idx);
				if( code === c_lb ){
					var node = _keywords[name](_getLex().trim(), root);
					node.line = line;
					node.file = file;
					if( !node.parent ){
						root.append(node);
...
//			_trace('loop');
			...
			if (state === void 0) {
				//  Состояние не определено, пытаемся понять

				if (code === c_acute) { // "`" -- script block
					tmp = root.newChild('x:script');
					last = idx + 1;

					while (input.charCodeAt(++idx) !== c_acute) {
					}

					tmp.value = _getLex().trim() + ';';
					_onTag(tmp);
				}
				else if (code === c_amp) {
					// "&" -- decl
					...
					// "|" -- текстовая нода
					_onText(_getLine(+1).trim().replace(/\|$/, ''));
				...
				else if (code === c_caret) {
					// ^ -- parent
					last = idx + 1;
					root = _close(root);
				}
				else if (code === c_slash) {
					// "//" -- Комментарий
					_getLine(); // skip line
				}
				else if (code === c_hash) {
					...
					last = idx;
					root = root.newChild('div');
					state = s_id;
				}
				else if (code === c_dot) {
					// "." -- начало ввода классов
					last = idx;
					root = root.newChild('div');
					state = s_class;
				}
				else if (code === c_gt || code === c_comma || code === c_plus) {
					// ">"  ","  "+"
					last = idx;
				}
				else if( code === c_lb ){ // "{"
			...
			// TAG or ATTR name
				...
				if (code === c_lp || prev < c_a) { // paser keywords
					name = input.substring(last, idx).trim();
					if (_keywords[name]) {
						last = idx;
						root = _addKeyword(name);
						state = void 0;
						continue;
					}
				}
				...
				else if (code === c_gt || code === c_plus || code === c_lb || code === c_caret) {
					// ">", "+", "{"
					state = s_block;
				}
				else if (code > c_end && code < c_a && code !== c_minus && (code < c_0 || code > c_9)) {
					_throw();
				}

				if (state !== s_tag) {
					root = root.newChild(_getLex().trim());
					name = root.name;
					...
					if (!/^[a-z_-][a-z0-9_-]*$/i.test(name)) {
						_throw('Invalid node name "' + name + '"', -name.length / 2);
					}
				}
			}
			...
			// #ID
				...
				if( code === c_dot || code <= c_end || code === c_lbr ){
					state = code === c_dot
								? s_class
								: (code === c_lbr ? s_tag_attrs : s_block)
							;
					root.attrs.id = '"'+_getLex(+1).trim()+'"';
					idx--;
				}
			}

			// ".foo.bar"
			else if (state === s_class) {
				if (
					code <= c_end ||
					code === c_pipe ||
					code === c_lb ||
					...
					// "[\r\n]", "|", "{", "["
					if (code === c_lb && input.charCodeAt(idx+1) === c_lb) {
						while (input.charCodeAt(idx++) !== c_rb) {
							// считываем экспрешон
						}
					} else {
						if (!root.attrs['class']) {
						...
						root.attrs['class'] += _getLex(+1).trim() + ' ';

						if (code !== c_dot) {
							root.attrs['class'] = root.attrs['class'].trim() + '"';

							attr = void 0; // @todo: Выяснить, откуда это и написать тест!
							state = code === c_lbr ? s_tag_attrs : s_block;
						}
					}
				}
			}
			...
			// "[attr=value]"
			else if( state === s_tag_attrs ){
				if( code === c_eq || code === c_rbr ){ // "=", "]"
					if( attr === void 0 ){
						attr = _getLex(+1);
					}
					...
					if( code === c_rbr ){ // "]"
						value = _getLex(+1);
						last = idx+1; // @todo: badddd practice
						root.attrs[attr] = value === "]" ? "true" : value;
						attr = void 0;

						if( input.charCodeAt(idx+1) !== c_lbr ){ // "["
							state = s_block;
						}
					}
				}
			...
			// "{" or "|"
				...
				if (code === c_pipe) { // "|" -- text node
					_onText(_getLine(+1).trim().replace(/\|$/, ''));
					last = idx;
					root = _close(root);
					state = void 0;
				}
				else if (code === c_plus || code === c_caret) { // "+", "^"
					state = void 0;
					root = _close(root, true);
			...
			// ATTR NAME
			else if (state === s_attr || state === s_attr_prop) {
				if (code === c_colon) { // ":"
					value = _getLex().trim();
					...
					if (state === s_attr) {
						...
						if (tmp !== void 0 && attr === 'class') {
							root.attrs[attr] = tmp + ' ';
						}
					}
					else {
						prop = value;
						root.attrs[attr][prop] = '';
					}

					state = s_attr_val;
				}
				else if (code === c_quote) {
					// " — ковычка, это 100% аттрибут!
					while (input.charCodeAt(++idx) !== c_quote) {
						// ..
					}
				}
				else if (code === c_rb) { // "}"
					...
					// todo: Нужно думать над валидацией
//					_badChar();
				}
			}

			// ATTR VALUE
			else if (state === s_attr_val) {
				if (code === c_lb) { // "{"
					tmp = root.attrs[attr];
					root.attrs[attr] = {};

					if (tmp !== '' && attr === 'class') {
						root.attrs[attr][tmp] = 1;
					}
					...
					if (prop === void 0) {
						if (attr === 'class') {
							root.attrs[attr] = (root.attrs[attr] + value).replace(/"\s"/, ' ');
						}
						else {
						...
						state = s_attr_prop;
						root.attrs[attr][prop] = value;
						prop = void 0;
					}

					if (code === c_rb) {
						if (state === s_attr_prop) {
							idx++; // skip char
						}
						attr = void 0;
						prop = void 0;
			...
			// "&{declaration}
			else if (state === s_decl) {
				if (code === c_eq) { // "="
					root = root.newChild('x:decl', {name: _getLex(+1).trim()});
					state = void 0;
				}
				else if (code === c_lb) { // "{"
					root = root.newChild('x:' + _getLex(+1).trim());
					last = ++idx;
					state = s_attr;
				}
				else if (code > c_end && prev <= c_end) {
					root.newChild('x:' + _getLex(+1).trim(), {'x-context': _getLine()});
					state = void 0;
				}
			}
		...
		if( root.name !== '#root' ){
//			console.log(root);
			root.exception('not closed');
//			_throw('"'+root.name+'" not closed');
		}
...
});define("compile", function(module, exports, require){(function () {
	"use strict";
	...
	var gid = 0,
		...
		_each = utils.each,
		contextScope = [],
		rtext = /text|password|tel|email|number/
	;


	function _parseMods (expr) {
		if (expr.indexOf('|') > -1) {
			var mods = ['', ''];

			_each(utils.matchAll(expr, /(?:\|\|)|(\|\s*([a-z]+[^|]+))/g), function (args) {
				if (args[0]) {
					var mod = args[0].trim().split(':');
					mods[0] = '__xmods.' + mod[0].replace(/\|\s*/, '') + '(' + mods[0];
					mods[1] = mods[1] + (mod.length > 1 ? ',' + mod.slice(1).join(',') : '') + ')';
					expr = expr.replace(args[0], '');
				}
			});

			return	mods[0] + expr + mods[1];
		}
	...
	function compile(node, pre, tags){
		return	compile.code(node, pre, tags, 0);
	}



	compile.code = function _code(node, pre, tags, $applyIdx){
		var
			  code
			, name = node.name
			, parent = node.parent
		...
		if (pre[name]) {
			pre[name](node, node.attrs);
		}


		// Attributes preprocessing
		_each(node.attrs, function (value, attr) {
			var fn = pre['@' + attr];
			if (fn !== void 0){
				fn(node, attr, value);
				node.bindAttrs = true;
		...
		// Declaration
		if (node.xprops) {
//			contextScope.push(node.xprops);
//			bindOpen += '\n(function '+node.fnName+' () {';
//			bindClose += '})();\n';

			/**/
			_each(node.xprops, function (expr, name) {
				bindingGetter += compile.tryCatch(node, '__xargs["' + name + '"] = ' + expr);
				try {
					new Function(expr+' = '+expr);
					bindingSetter += compile.tryCatch(node, expr + ' = __xargs["' + name + '"]');
				} catch (err){}
			});

			bindOpen += '__buf.b("x:decl",\n' +
				'function (__xargs) {' + bindingGetter + '; __xargs.__parent = ctx; },\n' +
				'function '+node.fnName+' (ctx, __pctx) {\n';
			bindClose += '}, "' + name.substr(2) + '", function (__xargs) {' + bindingSetter + '});';
			/**/
		}
		...
		for (i = 0, n = children.length; i < n; i++) {
			childrenCode += _code(children[i], pre, tags, $applyIdx);
		}
		...
		// Bind attrs
		if (node.bindAttrs && !binding.length) {
			bindOpen += '__buf.bindOpen("attrs");';
			bindClose += '__buf.bindClose("attrs");';
		}


		// Binding
		for (i = 0, n = binding.length; i < n; i++) {
			bind = node;
			attr = binding[i];
			...
			expr = compile.getExpr(bind, attr);
			bind.attrs[attr] = '__xbind' + i;

			bindingArgs += ',__xbind' + i;
			bindingGetter += compile.tryCatch(node, '__xargs[' + i + '] = ' + expr);
		}
...
//				parent.attrs.id = parent.attrs.id || '$'+(++gid);
//			}

			bindOpen += '__buf.b("' + name + '",\n' +
				'function (__xargs) {' + bindingGetter + '},\n' +
				'function (' + bindingArgs.substr(1) + '){\n';
		...
		// Декларации
		_each(node.decl, function (expr, name) {
			bindOpen += '__buf.bindDecl("' + name + '",function (__xargs) {' + compile.tryCatch(node, '__xargs[0] = ' + expr) + '});';
		});


		if( fn === void 0 ){
			code = tags['**'](name, node, node.attrs);
		}
		else {
			code = fn === '' ? ['', ''] : fn(node, node.attrs);
		}


		return	(bindOpen + code[0] + childrenCode + code[1] + bindClose)
			.replace(/"\)\n__buf\.s\("/g, '')
			.replace(/{;_/g, '{_')
		;
	};
	...
	compile.lex = function (val){
		return	JSON.stringify(val);
	};


	compile.str = function (val){
		return	'__buf.s(' + compile.lex(val) + ')\n';
	};


	compile.w = function (val){
		return	'__buf.w(' + val+ ')\n';
	};


	compile.variable = function (node, name, output){
		return	compile.expr(node, '__buf.w($'+name+(output ? ',"'+output+'"' : '')+');');
	};


	compile.tryCatch = function (node, tryExpr, catchExpr){
		var scope = contextScope[contextScope.length - 1], key;
		if (scope) {
			for (key in scope) {
				tryExpr = tryExpr.replace(new RegExp('\\bctx\.'+key+'\\b', 'g'), scope[key]);
			}
		}
		return	'try{'+tryExpr+'}catch(e){'+(catchExpr || '')+(node.file ? ';__xerr(e,"'+node.file+'",'+node.line+')}' : '}');
	};


	compile.expr = function (node, expr){
		var init = '', idx = 0;

		expr = expr.replace(/([\$@&])([\w-]+)/g, function (_, mod, name){
			var val = node.attrs[name];

			if( mod == '$' && !node.attrs.safe ){
				val   = compile.getExpr(node, name);
				name  = '__xval' + idx++;
				init += compile.tryCatch(node, name+'='+val, name+'=void 0');
				val   = name;
			} else if( mod == '@' ){
				val	= compile.lex(val);
			}

			return	val === void 0 ? 'undefined' : val;
		});
	...
	compile.getExpr = function (node, name, val){
		var expr = _parseMods(val === void 0 ? node.attrs[name] : val).replace(/(\w|\))->(\w)/g, '$1.attributes.$2');
		...
		try {
			...
			try {
				...
				node.exception(' attribute "'+name+'": ' + err.message+'\n---\n'+expr+'\n---');
			}
		}
	...
	compile.attr = function (name, value, node, dynamic) {
		var code = '', offset = 0;
		...
		if (value === null) {
			...
			if (expr.indexOf('->') !== -1) {
				code += '__buf.bindModel(' + expr.replace('->', ', "') + '");';
				if( !rtext.test(type) ){
					attr = 'checked';
				}
			}
				...
				code += compile.attr('on-' + evt, expr, node);
			}

			if (type == 'radio') {
				value = value +' == ' + compile.lex(node.attrs.value);
			}

			code += compile.attr(attr, '{{' + value + '}}', node, node.name === 'textarea');
		}
		else if (name.substr(0, 3) === 'on-') {
			code = '__buf.bindEvent("' + name.substr(3) + '",' +
				'function (el, evt) {var __xret;' +
				compile.tryCatch(node, '__xret = ' + value) +
				'; if (__xret && __xret.always) {' +
					'el.disabled = true;' +
					'__xret.always(function () { el.disabled = false; __ctx.$apply() });' +
				'} });';
		}
		else if (/{{/.test(value)) {
			var idx = 0, args = [], exprs = [];

			value.replace(/\{\{(.*?)\}\}/g, function (match, expr, i) {
				if (i > 0) {
					code += compile.str(value.substring(offset, i));
				}

				expr = compile.getExpr(node, name, expr);

				args.push('__xbind'+idx);
				exprs.push(compile.tryCatch(node, '__xargs['+idx+'] = ' + expr));

				code += compile.w(args[idx]);
			...
			if (offset < value.length) {
				code += compile.str(value.substr(offset));
			}

			code = ('__buf.bindAttr("'+name+'",\n' +
				'function (__xargs) {' + exprs.join('') + '},\n' +
				'function(' + args.join(',') + '){' + code + '}' + (dynamic ? ', true' : '') + ');');
		}
		else {
			if (typeof value !== 'string') {
				value += '';
			}
			code = compile.str(' '+name+'='+compile.lex(value));
		}
...
});define("Binding", function(module, exports, require){(function () {
	var gid = 0;
	...
	var nextElementSibling = utils.nextElementSibling;


	if (0 && document.createTreeWalker) {
		walk = function (el, callback) {
			if (el.nodeType === 8){
				callback(el);
			}
			else {
				var treeWalker = document.createTreeWalker(el, NodeFilter.SHOW_COMMENT, null, false);
	...
	Bindind.prototype = {
		...
		scan: function (startEl, endEl) {
//			console.time('scan');

			var node = this._root,
				wallker = function (comment) {
					if (comment.nodeValue.charAt(0) == '/') {
//						console.log(' - ', node, node.cursor, comment);
						var el = node.el.nextSibling;

						if (node.name === 'x:value' && comment.previousSibling === el) {
							// одна тестовая нода
							node.el = el;
						} else {
							node._el = comment;
...
//						console.log(' + ', node.children[node.cursor], node.cursor, comment);
						node = node.children[node.cursor++];
						node.el = comment;
					}
				}
			;
			...
			if (endEl === void 0) {
				walk(startEl, wallker);
			}
			else {
				while (startEl.nextSibling !== endEl) {
					walk(startEl = startEl.nextSibling, wallker);
				}
			}
...
//			console.timeEnd('scan');
		...
		each: function (iterator) {
			var index = this._index, i = 0, root, bind, el, _el;

			for (; index[i] !== void 0; i++) {
				root = this._root;
				this._root = bind = index[i];
				...
				if (bind.zombi === 1) {
					...
					if (bind.render !== void 0) {
						iterator(bind, el, _el);
					}

					// Атрибуты
					var entries = bind.entries, len;
					if (entries !== void 0) {
						len = entries.length;
						while (len--) {
							iterator(entries[len], el, _el);
						}
					}
				}
	...
	function _removeChildren (children) {
		var i = children.length, bind, entries, el;
		...
		while (i--) {
			...
			el = bind.el.nextElementSibling || nextElementSibling(bind.el);
			bind.remove && bind.remove(el);
	...
	// Exports
	module.exports = Bindind;
})();
});define("buffer", function(module, exports, require){(function () {
	"use strict";

	var
		  utils = require('utils')
		, compile = require('compile')
		, _attrBool = { autofocus: 1, autoplay: 1, async: 1, checked: 1, controls: 1, defer: 1, disabled: 1, hidden: 1, loop: 1, multiple: 1, open: 1, readonly: 1, required: 1, scoped: 1, selected: 1 }
		, _attrFix = {
			tabindex: "tabIndex",
			readonly: "readOnly",
			"for": "htmlFor",
		...
		, _isClient = utils.isClient
		, _useArray = _isClient && (window.opera || /MSIE/.test(navigator.userAgent))

		, _inArray = utils.inArray
		, _isEmptyObject = utils.isEmptyObject
		, _notEqual = utils.notEqual

		, _each = utils.each
		, _escapeHTML = utils.escapeHTML

		, _cssText = _isClient && ('cssText' in document.createElement('div').style)

		, Binding = require('Binding')
		, dummyDiv = document.createElement('div')

		, now = utils.now
		, nextElementSibling = utils.nextElementSibling
	;
	 ...
	 * @constructor
	 */
	var Buffer = function (el, decl) {
		this.el = el;
		this.data = _useArray ? [] : '';

		this.decl = [];
		this.events = [];
		this.binding = new Binding(true);
		this.applyFns = [];

		this._decl = decl;

		this._w = this.w;
		this.$apply = this.applyChanges.bind(this);
	};
	...
	Buffer.prototype = {
		...
		s: _useArray
			? function (val) { this.data.push(val); }
			: function (val) { this.data += val; },
		...
		w: function (val, output) {
			if (val != null) {
				if (output === 'text') {
					this.s(val);
				}
				else {
					this.s(_escapeHTML(val));
				}
			}
		},
		...
		each: function (data, fn, filter) {
			if (data) {
				var i = 0, n = data.length, item;

				if (n >= 0) {
					for (; i < n; i++) {
						item = data[i];
						if (filter === void 0 || filter(item, i)) {
							fn(item, i);
						}
					}
				} else {
					for (i in data) {
						item = data[i];
						if (data.hasOwnProperty(i) && (filter === void 0 || filter(item, i))) {
							fn(item, i);
						}
					}
				}
		...
		b: function (name, getter, render, extra, setter) {
			var args = [],
				bind,
				isDecl = name === 'x:decl',
				mnemo = isDecl ? extra : name,
				decl = isDecl && this._decl[mnemo] || '',
				xparent
			;
//			console.log(mnemo, decl);

			getter(args);

			if (this.bindOn) {
				if (name == 'x:each') {
					args[0] = _clone(args[0], true);
				}
				else if (name !== 'x:decl') {
					args = _clone(args, name == 'x:each');
				}

				xparent = args.__parent;
				delete args.__parent;

				this.s('<!--' + mnemo + '-->');
				this.binding.open(bind = {
					name: name, // тип связки
					args: args, // текущие аргументы
					getter: getter, // получение новых аргументов
					render: render, // рендер функция
					extra: extra, // доп параметры
					setter: setter, // сеттер данных
					update: decl.update,
					remove: decl.remove
				});
			}
			...
			// Foreach
			if (name === 'x:each') {
				this.each(args[0], render, extra);
			}
			// Declaration
			else if (name === 'x:decl') {
				render.call(this, args, xparent);
				decl.init && this.decl.push({ name: mnemo, bind: this.binding._root, args: [args] });
			}
			else {
				// Expressions, IF
				if (args.length > 1) {
		...
		bindDecl: function (name, getter) {
			var args = [], decl = this._decl[name];

			getter(args);

			if (decl.init) {
				this.decl.push({ name: name, bind: this.binding._root, args: args });
			}

			if (decl.update || decl.remove) {
				this.binding.entry({
					name: 'decl',
					args: _clone(args),
					getter: getter,
		...
		bindAttr: function (name, getter, render, dynamic) {
			var args = [],
				val;
		...
		bindEvent: function (name, handler, selector, bind) {
			if (selector && typeof selector !== 'string') {
				bind = selector;
				selector = bind;
			}
			...
			if (name.indexOf(' ') > -1) {
				...
				var i = name.length;
				while (i--) {
					this.bindEvent(name[i], handler, selector, bind);
				}
			} else {
				this.events.push({
					name: name,
					bind: bind || this.binding._root,
					handler: handler,
					selector: selector
				});
			}
		...
		bindModel: function (model, attr) {
			var _this = this;
			var onFocus = function (el) {
				var newVal,
					attrs = model.attributes,
					isText = /text|pass/i.test(el.type + el.tagName) || el.contentEditable,
					oldVal = attrs[attr],
					bind = this
				;
...
//					console.log('onChange');
				...
				function onInput (el, evt) {
//					console.log('onInput', el);
					newVal = el.contentEditable ? el.innerHTML : el.value;

					if( isText ){
						if( (oldVal !== newVal) && (evt === true || evt.keyCode === 13) ){ // enter
							attrs[attr] = oldVal;
						...
						el.checked && model.set(attr, newVal);
					}
					else {
						model.set(attr, el.checked);
					}
				}
...
//					console.log('onBlur');
					...
					onInput(el, true);
					model.off('all', onChange);

					el.removeEventListener('blur', onBlur.handleEvent);
					el.removeEventListener('keyup', onInput.handleEvent);
					el.removeEventListener('change', onInput.handleEvent);
				}

				console.log('onFocus:', isText);
				model.on('all', onChange);
				_this.bindEvent('blur', onBlur, bind);
				_this.bindEvent(isText ? 'keyup' : 'change', onInput, bind);
			};

			// Анктивация
			_this.bindEvent('focus', onFocus);
		},

		bindOpen: function (name) {
			this.s('<!--' + name + '-->');
			this.binding.open({});
		},

		bindClose: function (name) {
			this.s('<!--/' + name + '-->');
			this.binding.close();
		},
		...
		toString: _useArray
			? function () { return this.data.join('') }
			: function () { return this.data; },

		applyChanges: function (fn) {
			var _buf = this,
				timers = {
					all: now(),
					getter: 0,
			...
			this.binding.each(function (bind, el, _el) {
				var  name = bind.name
					, extra = bind.extra // filter или название атрибута
					, oArgs = bind.args
					, argsLen

					, getter = bind.getter
					, renderFn = bind.render
				...
				argsLen = nArgs.length;

				timers.getter += now() - ts;
				ts = now();

				if (name === 'x:each') {
					if (_notEqual(oArgs[0], nArgs[0]) ){
						changed = true;
						nArgs[0] = _clone(nArgs[0], true);
					}
				}
				else {
					var i = nArgs.length;
					while (i--) {
						if (_notEqual(oArgs[i], nArgs[i])) {
							changed = true;
							nArgs[i] = _clone(nArgs[i]);
						}
					}
				}
...
//				console.log(name, changed, nArgs[0] === oArgs[0]);


				if (changed) {
					// Чистим буфер и биндинг
					_buf.clear();
					_buf.binding.clear(bind);

					_buf.w = (name === 'attrs' || name === 'x:value') ? _buf.s : _buf._w;
					...
					if (name === 'x:each') {
						// Пока перерисовываем всё
						_buf.each(nArgs[0], renderFn);
					}
					else if (name === 'decl') {
						// Декларации
						bind.update && bind.update(el.nextElementSibling || nextElementSibling(el), nArgs[0], oArgs[0]);
						return;
					}
					else {
						// Микрооптимизация
						if (argsLen === 1) {
							renderFn.call(_buf, nArgs[0]);
						} else {
							renderFn.apply(_buf, nArgs);
						}
					}

					// Получим, что получилось :]
					var val = _buf.toString();

					timers.render += now() - ts;
					ts = now();


					// Атрибуты
					if (name === 'attrs') {
						el = el.nextSibling; // следующий узел и есть наш элемент
//						console.log(el, extra, val);
						...
						else if (_attrBool[extra] !== void 0) {
							el[_attrFix[extra] || extra] = (val !== '' && val !== 'false' && val !== '0' && val !== 'null');
						}
						else if (extra === 'style' && _cssText) {
							el.style.cssText = val;
						}
						else if (val === 'null') {
							el.removeAttribute(extra);
						}
						else {
							el.setAttribute(extra, val);
						}
					}
					// Обычное выражение: value, if, each
					else {
						if (el.nodeType === 3) {
							el.nodeValue = val;
						}
						else if (_replaceBetween(el, _el, val)) {
//							console.log('>', el, _buf.binding._root.cursor);
							_buf.binding.scan(el, _el);
						}
					}
...
//		console.groupEnd();
			...
			console.info(
				'all: %s, getter: %s, equal: %s, clear: %s, render: %s, dom: %s',
				(now() - timers.all).toFixed(3), timers.getter.toFixed(3), timers.equal.toFixed(3), timers.clear.toFixed(3), timers.render.toFixed(3), timers.dom.toFixed(3)
			);
		},

		applyEnd: function (scan) {
			if (scan === true) {
				this.binding.scan(this.el);
			}

			var _this = this, _decl = this._decl;


			// Инициализируем декларации
			_each(this.decl, function (decl, init) {
				var el = decl.bind.el;
				init = _decl[decl.name].init;
				init.call(_this, el.nextElementSibling || nextElementSibling(el), decl.args[0]);
			});


			// Вешаем слушателей
			_each(this.events, function (event) {
				var bind = event.bind,
					...
					handleEvent = function (evt) {
						...
						// Обработчик события
						handler.call(bind, evt.currentTarget, evt);

						// Обновляем данные (декларации)
						var parent = bind;
						do {
							parent.setter && parent.setter(parent.args);
				...
				// Обработчик события
				bind.el.nextSibling.addEventListener(event.name, handleEvent, false);
			});
		...
		destory: function () {
			this.events = [];
			this.binding.reset();
			_cleanup(this.el, this._decl, true);
		}
	};
	Buffer.prototype._w = Buffer.prototype.w;
	...
	function _createNode(fragment, html) {
		if (/^(<!--.*?-->)?<option/i.test(html)) {
			fragment.innerHTML = '<select>' + html + '</select>';
			fragment = fragment.firstChild;
		}
		else if (/^(<!--.*?-->)?<tr/.test(html)) {
			fragment.innerHTML = '<table><tbody>' + html + '</tbody></table>';
			fragment = fragment.firstChild.firstChild;
		}
		else if (/^(<!--.*?-->)?<td/.test(html)) {
			fragment.innerHTML = '<table><tbody><tr>' + html + '</tr></tbody></table>';
			fragment = fragment.firstChild.firstChild.firstChild;
		}
		else {
		...
		if (fragment.childNodes.length > 1) {
			...
			while (fragment.firstChild) {
				realFragment.appendChild(fragment.firstChild);
			}
			return realFragment;
		}
	...
	function _replaceBetween(startEl, endEl, html) {
		...
		if (html !== '') {
			parentEl.insertBefore(_createNode(dummyDiv, html), endEl);
			return true;
		}
	...
	function _replaceNode(refNode, newNode, parent) {
		if (parent === void 0) parent = refNode.parentNode;
	 ...
	 * @private
	 */
	function _clone(val, simple) {
		var res = val, key, idx = 0;

		if (val instanceof Array) {
			key = val.length;
			res = [];
			while (key--) {
				res[key] = simple ? val[key] : _clone(val[key]);
			}
		}
		else if (val instanceof Function) {
			res = val;
...
//				}
...
});define("xtpl", function(module, exports, require){(function () {
	"use strict";
	...
	var
		...
		_xtpl = _isClient && window.xtpl,
		_dummyId = _isClient ? location.pathname : __dirname,
		...
		_includeFile = function (node, attrs){
			var src = _dirname + attrs.src;
			var inc = xtpl.parse(utils.readFile(src+'?'+Math.random()), src);
			node.replace(inc.children);
		},
		...
		_onParser = {
			...
			'x:text': function (node, attrs){
				var text = node.first ? node.first.value : attrs.value;
				return	text;
			},
			'x:space': ' ',
			'x:enter': '\n',
			'x-space': ' ',
			'x-enter': '\n',
			'include': _includeFile,
			'x:include': _includeFile
		},

		_each = utils.each,
		_extend = utils.extend,

		_compileTagProcessorSetter = function (tag, fn){
			var tags = {}, decl = this;
			...
			_each(tags, function (fn, name){
				if( fn.pre || fn.code ){
					if( fn.pre ){ _compileTagProcessorSetter.call(_pre, name, fn.pre); }
					if( fn.code ){ _compileTagProcessorSetter.call(_tags, name, fn.code); }

					var allowChildren = fn.children;
					if( allowChildren ){
						_onParser[name] = function (node){
							var children = node.children, i = node.length;

							if( allowChildren ){
								if( i === 0 ){
									node.exception('can not be empty');
								}

								while( i-- ){
									if( allowChildren.indexOf(children[i].name) === -1 ){
										node.exception('can contain only: '+allowChildren.replace(/\s/g, ', '));
									}
								}
							}
						};
					...
					// x:value, x-attr, x-repeat
					decl[name] =
					decl[name.replace(':', '-')] = fn;
				}
			});
		},


		_declProps = function (node, attrs) {
			node.xprops = {};

			_each(attrs, function (value, name) {
				if (/^x-/.test(name)) {
					node.xprops[utils.toCamelCase(name.substr(2))] = value.replace(/(\w|\))->(\w)/g, '$1.attributes.$2');
				} else {
					if (typeof value === 'string') {
						value = value.replace(/\bctx\./g, '__pctx.');
					}

					node.first.attrs[name] = value;
				}
			});
		}
	;
	...
	var xtpl = {
		 ...
		 * @return {Node}
		 */
		parse: function (str, file){
			return	xparser(str, { file: file || _dummyId }, _onParser);
		},
		 ...
		 * @returns {Function}
		 */
		compile: function (str, id){
			var fn = _cache[id], code;

			if( fn === void 0  ){
				if (typeof str === 'string') {
					str = xtpl.parse(str, id);
				}

				try {
					code = compile(str, _pre, _tags);
...
//					prompt('?', code);
					fn = (new Function('return function(__buf){'+compile.str('<pre>'+err.message+'\nline: '+err.line+'\nfile: '+err.file+'</pre>')+'return __buf.toString()}'))();
					console.error(err);
				}
		 ...
		 * @returns {Object}
		 */
		bind: function (el, html, ctx){
			var fn = typeof html === 'string' ? xtpl.compile(xtpl.parse(html)) : html;
			var buf = new Buffer(el, _decl);
			...
			ctx.$clear = function (id){
				clearTimeout(id);
			};

			ctx.$apply = buf.$apply;

			el.innerHTML = fn(buf, ctx);
			buf.applyEnd(true);
		...
		decl: function (name, decl) {
			...
			_each(decls, function (decl, name) {
				if (typeof decl === 'function') {
					decl = { init: decl, update: decl };
				}

				decl = _decl[name] = _extend(_decl[name], decl);
				xname = name.replace('x:', 'x-');
				_decl[xname] = decl;

				if (decl.tpl) {
					xtpl.tag('x:' + name, {
						pre: function (node, attrs) {
							var tpl = typeof decl.tpl === 'string'
									? (decl.tpl = xtpl.parse(decl.tpl).first)
									: decl.tpl.clone(),
								xcontent = tpl.find('x:content')[0] || tpl.find('x-content')[0]
							;

							if (xcontent) {
								xcontent.appendArray(node.children);
							}

							_each(tpl.find('x:' + name), function (node) {
								node.name = 'x:decl';
								node.declName = name;
								node.newChild('x:decl-call', { name: name });
								_declProps(node, node.attrs);
							});

							node.name = 'x:decl';
							node.fnName = '__x' + (node.declName = name.replace(/[^a-z0-9]/g, ''));
							node.empty().append(tpl);
							node.bindAttrs = false;

							_declProps(node, attrs);

//							if (node.attrs['x-context']) {
//								node.xprops = node.attrs['x-context'];
//								delete node.attrs['x-context'];
//							}

//							if (decl.init || decl.update || decl.remove) {
//								node.bindDecl(name, 'ctx');
//							}
						},
						code: function () {
							return ['', ''];
					...
					_pre['@' + xname] = _pre['@' + name] = function (node, name, expr) {
						delete node.attrs[name];

						node.bindAttrs = true;

						if (decl.compile) {
							decl.compile(node, expr);
						}

						if (decl.init || decl.update || decl.remove) {
							node.bindDecl(name, expr);
						}
					};
				}
			});
		...
		pre: _compileTagProcessorSetter.bind(_pre),
		tag: _compileTagProcessorSetter.bind(_tags)

	};


	_onParser['x:decl'] = _onParser['x-decl'] = function (node, attrs) {
		if (attrs.name) {
			node.remove();
			xtpl.decl(node.attrs.name, { tpl: node.first });
		}
	};


	xtpl.pre({
		'@x:model': function (node) {
			// блин, так правильно начиниает работать, магия
		}
	});
	...
	xtpl.tag({
		'#root': function (){
			return [
				  'function _(__buf,ctx){var'
				+ ' __xerr=xtpl.utils.error'
				+ ',__xlog=xtpl.utils.print'
				+ ',__xnotEq=xtpl.utils.notEqual'
				+ ',__xclone=xtpl.utils.simpleClone'
				+ ',__xmods=xtpl.mods'
				+ ',xutils=xtpl.utils'
				+ ',__xval,__xval0,__xval1,__xval2,__xval3,__xval4'
				+ ',__$apply0=__buf.$apply'
				+ ',__ctx=ctx'
				+ ',model=ctx.model'
		...
		'eval': {
			...
			code: function (node, attrs) {
				return ['xtpl.compile(' + attrs['x-source'] + ')(__buf, ctx);', ''];
			}
		},

		'x:scope': function (node, attrs) {
			return [';(function(ctx){', '})(' + attrs.ctx + ');'];
		},

		'x:value': {
			pre: function (node, attrs){
				attrs.data = attrs.data || node.value || node.first.remove().value;
				(attrs.bind !== false) && node.bind('data');
			},
			code: function (node, attrs) {
				return [compile.variable(node, 'data', attrs.output), ''];
			}
		},

		'x:script': {
			pre: function (node, attrs) {
				attrs.source = node.value || node.first.remove().value;
			},
			code: function (node, attrs) {
				return [compile.tryCatch(node, attrs.source), ''];
			}
		},

		'x:if': {
			pre: function (node, attrs){
				attrs.test = '!!('+attrs.test+')';
				node.bind('test');
				node.braces = 1;
			},
			code: function (node, attrs) {
				return [compile.expr(node, 'if($test){'), (new Array(node.braces+1)).join('}')];
			}
		},
		...
		'x:else': {
			...
			code: function (node, attrs) {
				...
				return ['} else {\n' + compile.expr(node, expr), ''];
			}
		},

		'x:each': {
			code: function (node, attrs){
				return ['__buf.b("x:each", function(__xargs){'
					  + compile.tryCatch(node, '__xargs[0]=' + attrs.data)
					  + compile.expr(node, '},function(&as,&key){')
					, compile.expr(node, '},&filter);')
				];
			}
		},
		...
		'textarea': {
			pre: function (node, attrs) {
				var expr = attrs['x-model'] || attrs.value;
				if (expr) {
					node.bindAttrs = true;
					attrs['x-model'] = expr;
					delete attrs.value;
					node.newChild('x:value', { data: expr, bind: false });
				}
			}
		},

		'x:bind': function (node, attrs) {
			var i = this.idx++;

			return [
				'var __' + i + 'bindOn = __buf.bindOn;' +
				'__buf.bindOn = ' + attrs.state + ';',

				'__buf.bindOn = __' + i + 'bindOn;'
			];
		}.bind({ idx: 0 }),


		'**': {
			code: function (name, node, attrs) {
				var
					  _attrs = ''
					, key
					, val
				...
				for (key in attrs) {
					val = attrs[key];
					_attrs += compile.attr(key, val, node);
				}

				return [
					compile.str('<' + name) + _attrs + compile.str(shorty ? '/>' : '>'),
					shorty ? '' : compile.str('</' + name + '>')
				];
			}
		}
	});
	...
	xtpl.decl({
		'x:show': {
			compile: function (node, expr) {
				...
				attrs.style = (attrs.style || '') + ";{{" + expr + " ? '' : 'display: none;'}}";
			}
		},
		...
		'x:fade-to': function (el, val) {
			el.style.opacity = val | 0;
		},

		'x:autofocus': function (el, val) {
			if (val) {
				setTimeout(function () {
					el.focus();
					el.selectionStart = el.selectionEnd = el.value ? el.value.length : 0;
				}, 1);
			}
		}
	});
	...
	// Preset mods
	xtpl.mod('css', function (val, state) {
		return state ? val : '';
	});
...
define('node_modules/requirejs-text/text.js',['module'], function (module) {
    'use strict';
...
define('node_modules/requirejs-text/text.js!ui/nav/nav.xtpl',[],function () { return '.nav > for item in ctx.props.items {\n\ta.&__item[href="#!{{ctx.props.route | routeUrl:item}}"] {\n\t\tclass: { &_active: ctx.props.active == item.id }\n\n\t\ti.fa.fa-{{ item.icon }} + span | {{ item.text }}\n\n\t\tif item.badge {\n\t\t\tspan.&__badge | {{ item.badge }}\n\t\t}\n\t}\n\n\t// Подменю\n\t.&__subitems {\n\t\tclass: { &_close: !ctx.props.expanded[item.id] }\n\n\t\tfor item in item.items {\n\t\t\ta.&__item[href="#!{{item.url}}"] {\n\t\t\t\tclass: { &_active: ctx.props.active == item.id }\n\t\t\t\ton-click: ctx.props.active = item.id;\n\n\t\t\t\ti.fa.fa-{{ item.icon }} + span | {{ item.text }}\n\n\t\t\t\tif item.badge {\n\t\t\t\t\tspan.&__badge | {{ item.badge }}\n\t\t\t\t}\n\t\t\t}\n\t\t}\n\t}\n}\n';});

define('ui/nav/nav',['require','css!./nav','ui/element','text!./nav.xtpl'],function (require) {
	'use strict';
...
define('node_modules/require-css/css.js!ui/caption/caption',[],function(){});

define('node_modules/requirejs-text/text.js!ui/caption/caption.xtpl',[],function () { return '.caption | {{ctx.props.text || \'\'}}&nbsp;\n';});

define('ui/caption/caption',['require','css!./caption','ui/element','text!./caption.xtpl'],function (require) {
	require('css!./caption');
...
define('node_modules/require-css/css.js!ui/toolbar/toolbar',[],function(){});

define('node_modules/requirejs-text/text.js!ui/toolbar/toolbar.xtpl',[],function () { return '.toolbar {\n\tclass: { &_hidden: ctx.props.hidden }\n\n\tfor item in ctx.props.items {\n\t\t.&__item {\n\t\t\ton-click: item.onclick(item);\n\t\t\ti.fa.fa-{{ item.icon }} + span | {{ item.text }}\n\t\t}\n\t}\n}\n';});

define('ui/toolbar/toolbar',['require','css!./toolbar','ui/element','text!./toolbar.xtpl'],function (require) {
	require('css!./toolbar');
...
define('node_modules/require-css/css.js!ui/letter/letter',[],function(){});

define('node_modules/requirejs-text/text.js!ui/letter/letter.xtpl',[],function () { return '.letter {\n\t.caption | {{ctx.model.get(\'subject\')}}\n\n\t.&__info | от {{ ctx.model.get(\'correspondents.from.0.name\') || ctx.model.get(\'correspondents.from.0.email\') }}\n\t.&__body | {* ctx.model.get(\'body.html\').replace(/<base.*?>/g, \'\') *}\n}\n\n';});

define('ui/letter/letter',['require','css!./letter','ui/element','text!./letter.xtpl'],function (require) {
	require('css!./letter');
...
define('pilot/mixin/xtpl',['xtpl', 'pilot/mixin/view'], function (/** xtpl */xtpl, view) {
	// Export
...
define('node_modules/requirejs-text/text.js!ui/layout/layout.xtpl',[],function () { return '.layout {\n\tclass: {\n\t\tlayout_3: ctx.request.is("#message")\n\t}\n\n\t.&__1 { x-route: "#:folders" }\n\t.&__2 { x-route: "#messages" }\n\n\tif ctx.request.is("#message") {\n\t\t.&__3 { x-route: "#message" }\n\t}\n}\n';});

define('sitemap',['require','ui/nav/nav','ui/caption/caption','ui/toolbar/toolbar','ui/datalist/datalist','ui/letter/letter','Action','mail/Folder','mail/Message','mediator/action','mediator/selection','service/auth','pilot/mixin/xtpl','text!ui/layout/layout.xtpl','pilot/mixin/xtpl','pilot/mixin/xtpl','pilot/mixin/xtpl'],function (require) {
	'use strict';
...
('.nav {\n}\n\t.nav__item {\n\t\tcolor: #637074;\n\t\tdisplay: block;\n\t\tfont-size: 16px;\n\t\tfont-family: \'PT Sans\', sans-serif;\n\t\ttext-decoration: none;\n\t\tpadding: 15px 20px;\n\t\theight: 20px;\n\t\twhite-space: nowrap;\n\t\toverflow: hidden;\n\t\ttext-overflow: clip;\n\t}\n\n\t\t.nav__item .fa {\n\t\t\tmargin-right: 10px;\n\t\t}\n\n\t\t.nav__subitems .nav__item {\n\t\t\tpadding-left: 30px;\n\t\t}\n\n\t.nav__item_active {\n\t\tbackground: #fff;\n\t}\n\n\t.nav__subitems_close {\n\t\tdisplay: none;\n\t}\n\n\t.nav__badge {\n\t\tcolor: #fff;\n\t\tfloat: right;\n\t\tpadding: 5px 5px 4px;\n\t\tfont-size: 12px;\n\t\tbackground: #D04F3A;\n\t\tborder-radius: 100%;\n\t\tmin-width: 15px;\n\t\ttext-align: center;\n\t}\n.caption {\n\tcolor: #323B3D;\n\tfont-size: 40px;\n\tmargin-top: 25px;\n\tmargin-bottom: 35px;\n\tfont-family: \'Roboto\', sans-serif;\n\toverflow: hidden;\n\twhite-space: nowrap;\n\ttext-overflow: ellipsis;\n}\n.toolbar {\n\tleft: 22.35%;\n\twidth: 70%;\n\tbottom: 0;\n\tposition: fixed;\n\tpadding: 10px 15px;\n\tbackground-color: #fff;\n\tborder-radius: 5px 5px 0 0;\n\tbox-shadow: 0 0 5px rgba(0,0,0,.7);\n\toverflow: hidden;\n\ttransition: all .2s;\n}\n\t.layout_3 .toolbar {\n\t\tleft: 20%;\n\t\twidth: 30%;\n\t}\n\n\t.toolbar_hidden {\n\t\theight: 0;\n\t\topacity: 0;\n\t\tpadding: 0;\n\t}\n\n\t.toolbar__item {\n\t\tcursor: pointer;\n\t\ttransition: color .2s;\n\t\tmargin-right: 10px;\n\t\tdisplay: inline-block;\n\t}\n\t.toolbar__item:hover {\n\t\tcolor: #D04F3A;\n\t}\n\n\t.toolbar__item i {\n\t\tfont-size: 18px;\n\t\tmargin-right: 5px;\n\t\tvertical-align: middle;\n\t}\n.datalist {\n\tfont-family: \'PT Sans\', sans-serif;\n\tmargin-left: -30px;\n}\n\t.datalist__item {\n\t\tdisplay: block;\n\t\tpadding: 5px 0 5px 30px;\n\t\tmargin-bottom: 1px;\n\t\tborder-radius: 5px;\n\t\tposition: relative;\n\t\ttransition: background-color .2s;\n\t}\n\n\t.datalist__item_active {\n\t\tbackground-color: #E9EDF0;\n\t}\n\n\t.datalist__item_selected {\n\t\tbackground-color: #E0E6EA;\n\t}\n\n\t.datalist__item > * {\n\t\toverflow: hidden;\n\t\tdisplay: inline-block;\n\t\tvertical-align: middle;\n\t\twhite-space: nowrap;\n\t\ttext-decoration: none;\n\t}\n\n\t.datalist__unread {\n\t\tcursor: pointer;\n\t\twidth: 10px;\n\t\theight: 10px;\n\t\tborder: 2px solid #45BF7B;\n\t\tborder-radius: 100%;\n\t\tposition: absolute;\n\t\ttop: 14px;\n\t\tleft: 10px;\n\t\topacity: 0;\n\t\ttransition: opacity .2s;\n\t\t-webkit-user-select: none;\n\t\tuser-select: none;\n\t}\n\n\t.datalist__item:hover .datalist__unread {\n\t\topacity: 0.4;\n\t}\n\n\t.datalist__item:hover .datalist__unread:hover {\n\t\topacity: 1;\n\t}\n\n\t.datalist__item:hover .datalist__unread_yes,\n\t.datalist__unread_yes {\n\t\topacity: 1;\n\t\tbackground: #45BF7B;\n\t}\n\n\t.datalist__ctrl {\n\t\tcursor: pointer;\n\t\topacity: 0;\n\t\ttransition: opacity .2s, color .2s;\n\t\tmargin-left: 10px;\n\t\tfont-size: 20px;\n\t}\n\t\t.datalist__ctrl:hover {\n\t\t\tcolor: #D04F3A;\n\t\t}\n\n\t.datalist__item:hover .datalist__ctrl {\n\t\topacity: 1;\n\t}\n\n\n\t.datalist__ava {\n\t\twidth: 30px;\n\t\theight: 30px;\n\t\tborder-radius: 100%;\n\t\tmargin-right: 10px;\n\t\tbackground-size: cover;\n\t\t-webkit-user-select: none;\n\t\tuser-select: none;\n\t}\n\t\t.datalist__ava .fa {\n\t\t\topacity: 0;\n\t\t\tcolor: #000;\n\t\t\tcursor: pointer;\n\t\t\tfont-size: 36px;\n\t\t\ttransform: translateY(-3px);\n\t\t\ttransition: opacity .2s;\n\t\t}\n\t\t\t.datalist__ava:hover .fa {\n\t\t\t\topacity: 0.5;\n\t\t\t}\n\n\t.datalist__user {\n\t\tcolor: #323B3D;\n\t\twidth: 20%;\n\t}\n\n\t.datalist__subj {\n\t\tcolor: #858C8E;\n\t\twidth: 50%;\n\t\tmargin-left: 5%;\n\t\ttext-overflow: ellipsis;\n\t}\n.letter {\n\tfont-family: \'PT Sans\', sans-serif;\n}\n\t.letter__info {\n\t\tmargin-top: -30px;\n\t}\n\n\t.letter__body {\n\t\tmargin-top: 20px;\n\t}\n');
